...
현재 Application Factory와 Health Endpoint에 Room, Game 및 Vote·Turn Pure Domain과 Member·Session, Lobby·Room Headless HTTP를 연결했습니다. Room Domain은 공개·비공개 접근 경계, 방·참가자·팀·Ready·방장 승계·시작 조건을 판단합니다. Game Domain은 15×15 Board·좌표·Move·Renju 금수·승패에 더해 확정 이탈 기반 몰수·시스템 무효 종료와 Elo 갱신 계획을 담당합니다. Vote Domain은 현재 팀의 최종 유효표, 마감 후보, 외부 동률 선택 입력, Pass와 연속 무투표 공동 패배를 외부 Provider 없이 판단합니다. Member 가입·인증은 [Member Identity Application 경계](docs/member-identity.md), Lobby·Room API와 참가 식별은 [Lobby·Room HTTP 경계](docs/lobby-room-http.md), Game Application Port와 MariaDB Adapter는 [Game Persistence 경계](docs/game-persistence.md)를 따릅니다. Redis 공통 규격과 서버측 Session Lifecycle은 [Redis Session Adapter 경계](docs/redis-session.md)를 따르고, Room·Ready·연결 세대·단절 유예·방장 승계는 [Redis Room Runtime Adapter 경계](docs/redis-room-runtime.md), Game·Turn·Vote 마감과 Resolver Lease는 [Redis Vote Runtime Adapter 경계](docs/redis-vote-runtime.md)를 따릅니다. 실제 Provider 연결은 각각 별도 Gate로 유지합니다. HTTP·WebSocket, 상태, Redis·MariaDB 기준은 [Application MVP 구현 기준](../docs/mvp-implementation-baseline.md)을 따릅니다.
