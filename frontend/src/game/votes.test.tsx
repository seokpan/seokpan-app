...
describe("server vote tally presentation", () => {
  ...
  it("has no percentage for zero votes and supports fractions", () => {
    ...
    expect(summarizeVotes([{ coordinate: "A1", count: 1 }, { coordinate: "B1", count: 2 }], 3).rows.map(v => v.label)).toEqual(["66.7%", "33.3%"]);
  });
  it("shows all candidates, distinguishes the viewer and removes old turn badges", () => {
    const rows = summarizeVotes([{ coordinate: "H8", count: 1 }, { coordinate: "G7", count: 1 }], 2).rows;
    const { rerender } = render(<Board cells={[]} chosen="H8" votes={rows} />);
    expect(screen.getByRole("button", { name: "H8 빈 자리, 내 투표, 1표 50.0%" })).toBeInTheDocument();
  ...
  it("recalculates after vote removal and disconnection without choosing a winner", () => {
    const before = summarizeVotes([{ coordinate: "H8", count: 1 }, { coordinate: "G7", count: 1 }], 3);
