...
const member = { actor_type: "MEMBER", actor_id: "1", display_name: "돌하나", csrf_token: csrf,
...
const room = { room_id: "room-one", name: "한 수 같이 둬요", visibility: "PUBLIC", password_required: false,
  participant_count: 2, max_participants: 4, minimum_ready: 4, vote_seconds: 15, status: "WAITING", state_version: 4 };
...
function mount(fetcher: typeof fetch, path = "/", strict = false) {
  const transport: typeof fetch = (url, options) => String(url).startsWith("/api/v1/rankings")
    ? Promise.resolve(json({ items: [], me: { member_id: 1, nickname: "돌하나", rating: 1000, wins: 0, draws: 0, losses: 0, games_played: 0, rank: null }, offset: 0, limit: 1, has_more: false }))
    : fetcher(url, options);
  const services = createSessionServices(new ApiClient(transport), () => ({ readyState: 0, onmessage: null, onclose: null, onerror: null, close: vi.fn() }));
  const element = <MemoryRouter initialEntries={[path]}><App services={services} /></MemoryRouter>;
...
describe("authentication and lobby screens", () => {
  it("does not carry logout feedback into the registration dialog", async () => {
    ...
    const fetcher = vi.fn<typeof fetch>(async (url, options) => {
      ...
      if (url === "/api/v1/session" && options?.method === "DELETE") { loggedOut = true; return new Response(null, { status: 204 }); }
  ...
  it("separates registration inputs and clears both forms on repeated open/close", async () => {
    ...
    for (let cycle = 0; cycle < 3; cycle++) {
      ...
      expect(within(dialog).getByLabelText("비밀번호")).toHaveAttribute("autocomplete", "section-signup new-password");
  ...
  it("logs in once, recovers the new Cookie, and shows validated server rooms", async () => {
    const fetcher = vi.fn<typeof fetch>().mockResolvedValueOnce(anonymous())
      .mockResolvedValueOnce(json(member)).mockResolvedValueOnce(json(member))
    ...
    expect(fetcher.mock.calls.map(call => call[0])).toEqual([
      "/api/v1/session/csrf", "/api/v1/sessions/member", "/api/v1/session/csrf", "/api/v1/lobby/snapshot",
    ]);
    expect(JSON.parse(String(fetcher.mock.calls[1][1]?.body))).toEqual({ login_id: "member01", password: "pass word12" });
  ...
  it("creates one Guest on double activation and uses recovered CSRF for logout", async () => {
    ...
    const fetcher = vi.fn<typeof fetch>().mockResolvedValueOnce(anonymous())
      ...
      .mockResolvedValueOnce(json(guest)).mockResolvedValueOnce(json({ rooms: [], stream_version: 1 }))
      .mockResolvedValueOnce(new Response(null, { status: 204 })).mockResolvedValueOnce(anonymous());
    mount(fetcher);
    ...
    fireEvent.click(screen.getByRole("button", { name: "로그아웃" }));
    await screen.findByRole("button", { name: "로그인" });
    expect(fetcher.mock.calls.filter(call => call[0] === "/api/v1/sessions/guest")).toHaveLength(1);
  ...
  it("registers with trimmed nickname but does not automatically log in", async () => {
    const fetcher = vi.fn<typeof fetch>().mockResolvedValueOnce(anonymous())
      .mockResolvedValueOnce(json({ member_id: 1, login_id: "member01", nickname: "새회원", rating: 1000 }, 201))
      .mockResolvedValueOnce(anonymous());
    mount(fetcher);
    fireEvent.click(await screen.findByRole("button", { name: "회원가입" }));
    fill(); fireEvent.change(screen.getByLabelText("닉네임"), { target: { value: " 새회원 " } });
  ...
  it("rejects invalid input before sending and does not reveal the password", async () => {
    ...
    mount(fetcher); await screen.findByRole("button", { name: "로그인" });
    fill("UPPER", "secret-password"); fireEvent.click(screen.getByRole("button", { name: "로그인" }));
  ...
  it("recovers an uncertain login without repeating it", async () => {
    const fetcher = vi.fn<typeof fetch>().mockResolvedValueOnce(anonymous())
      ...
      .mockResolvedValueOnce(json(member)).mockResolvedValueOnce(json({ rooms: [], stream_version: 1 }));
    mount(fetcher); await screen.findByRole("button", { name: "로그인" });
    fill(); fireEvent.click(screen.getByRole("button", { name: "로그인" }));
    await screen.findByText("아직 열린 방이 없습니다.");
    expect(screen.getByText(/요청이 처리되었을 수 있으니/)).toBeInTheDocument();
    expect(fetcher.mock.calls.filter(call => call[0] === "/api/v1/sessions/member")).toHaveLength(1);
  });

  it("does not disguise a recovery service error as a missing login", async () => {
    const fetcher = vi.fn<typeof fetch>().mockResolvedValueOnce(json({}, 503)).mockResolvedValueOnce(anonymous());
    mount(fetcher);
  ...
  it("renders untrusted names as text and keeps full/playing rooms visible", async () => {
    ...
    const fetcher = vi.fn<typeof fetch>().mockResolvedValueOnce(json(member))
      .mockResolvedValueOnce(json({ stream_version: 7, rooms: [{ ...room, name, status: "PLAYING", participant_count: 4 }] }));
    mount(fetcher);
  ...
  it("recovers after a lobby authentication rejection without retrying the list", async () => {
    const fetcher = vi.fn<typeof fetch>().mockResolvedValueOnce(json(member))
      .mockResolvedValueOnce(anonymous()).mockResolvedValueOnce(anonymous());
    mount(fetcher);
    await screen.findByRole("button", { name: "로그인" });
    expect(fetcher.mock.calls.filter(call => call[0] === "/api/v1/lobby/snapshot")).toHaveLength(1);
  });

  it("survives StrictMode recovery cleanup without issuing Guest sessions", async () => {
    const fetcher = vi.fn<typeof fetch>().mockImplementation(async path => path === "/api/v1/session/csrf"
      ? anonymous() : json({}));
    mount(fetcher, "/", true);
    await screen.findByRole("button", { name: "로그인" });
    expect(fetcher.mock.calls.every(call => call[0] === "/api/v1/session/csrf")).toBe(true);
  ...
  it("offers manual refresh after malformed list data instead of showing fake empty success", async () => {
    ...
    mount(fetcher); await screen.findByRole("alert");
  ...
  it("provides a safe unknown route without inventing a game page", async () => {
    ...
    await waitFor(() => expect(screen.getByRole("link", { name: "시작 화면으로 돌아가기" })).toBeInTheDocument());
  });
  ...
  it("does not reset a newer recovery when an unmounted auth request finishes late", async () => {
    ...
    expect(services.recovery.getSnapshot()).toMatchObject({ phase: "ready", identity: { actor_id: "1" } });
    expect(fetcher).toHaveBeenCalledTimes(3);
  });
  ...
  it("shows duplicate registration feedback and does not repeat the registration", async () => {
    const fetcher = vi.fn<typeof fetch>().mockResolvedValueOnce(anonymous())
      ...
      .mockResolvedValueOnce(anonymous());
    mount(fetcher); fireEvent.click(await screen.findByRole("button", { name: "회원가입" }));
    fill(); fireEvent.change(screen.getByLabelText("닉네임"), { target: { value: "새회원" } });
    fireEvent.click(screen.getByRole("button", { name: "가입하기" }));
    const dialog = screen.getByRole("dialog", { name: "Member 회원가입" });
