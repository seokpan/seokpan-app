...
export class PresenceStream {
  ...
  #publish(view: PresenceView) {
    ...
    this.#view = view; this.#listeners.forEach(fn => fn());
  }
  stop = () => {
    clearTimeout(this.#timer); this.#timer = undefined; this.#challenge = null;
    const socket = this.#socket; this.#socket = null;
    if (socket) { socket.onmessage = null; socket.onerror = null; socket.onclose = null; socket.close(); }
  };
  #fail = () => { this.stop(); this.#publish({ phase: "unavailable", count: null }); };
  #watch() {
    clearTimeout(this.#timer);
    this.#timer = setTimeout(this.#fail, 10_000);
  }
  start = () => {
    this.stop(); this.#publish({ phase: "checking", count: null });
    this.#timer = setTimeout(() => this.#connect(), 0);
  };
  #connect() {
    let socket: Socket;
    try { socket = this.factory("/ws/v1/presence"); }
    catch { this.#fail(); return; }
    this.#socket = socket; this.#watch();
    socket.onmessage = event => {
      ...
      try {
        ...
        if (data.schema_version !== 1 || typeof data.challenge !== "string" || !uuid.test(data.challenge)) throw new Error();
        if (data.event_type === "presence.ping") {
          if (Object.keys(data).length !== 3 || this.#challenge !== null || !socket.send) throw new Error();
          ...
          if (Object.keys(data).length !== 4 || this.#challenge !== data.challenge
            || !Number.isSafeInteger(data.online_users) || Number(data.online_users) < 1) throw new Error();
          this.#challenge = null; this.#watch();
          this.#publish({ phase: "ready", count: Number(data.online_users) });
        } else throw new Error();
      } catch { this.#fail(); }
    };
    socket.onerror = socket.onclose = () => { if (socket === this.#socket) this.#fail(); };
  }
}
