...
function validBootstrap(value: unknown): value is components["schemas"]["CsrfBootstrapResponse"] {
  ...
  return (record.actor_type === "MEMBER" || record.actor_type === "GUEST")
    && typeof record.actor_id === "string" && record.actor_id.length > 0
    && typeof record.display_name === "string" && record.display_name.length > 0
    && typeof record.csrf_token === "string" && /^[A-Za-z0-9_-]{32,256}$/.test(record.csrf_token)
    && typeof record.absolute_expires_at_ms === "number"
    && Number.isSafeInteger(record.absolute_expires_at_ms) && record.absolute_expires_at_ms > 0
    && nullableId(record.room_id) && nullableId(record.participant_id)
...
export class SessionRecovery {
  ...
  hold(): () => void {
    this.#holds++;
    let released = false;
    return () => { if (!released) { released = true; this.#holds--; } };
  }
  ...
  reset(preserveView = false): void {
    ...
    this.#publish(preserveView && this.#view.phase === "ready"
      ? { ...this.#view, checking: true } : { phase: "unknown" });
  }
  ...
  async #load(generation: number, signal: AbortSignal): Promise<void> {
    try {
      const result: unknown = await this.api.request("/api/v1/session/csrf", "post", { body: {}, signal });
      if (generation !== this.#generation) return;
      if (!validBootstrap(result)) throw new ApiFailure("invalid-response", 200, "INVALID_SESSION_RESPONSE");
      ...
      const identity: SessionIdentity = Object.freeze({
        ...
        display_name: result.display_name, absolute_expires_at_ms: result.absolute_expires_at_ms,
        room_id: result.room_id ?? null, participant_id: result.participant_id ?? null,
      });
      ...
      if (generation !== this.#generation) return;
      this.api.setCsrf(null);
      this.#publish(error instanceof ApiFailure && error.status === 401
        ? { phase: "anonymous" }
        : { phase: "error", error: error instanceof ApiFailure ? error : new ApiFailure("invalid-response") });
    } finally {
