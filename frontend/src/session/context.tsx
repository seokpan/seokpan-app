...
export function createSessionServices(api = new ApiClient(), socketFactory: SocketFactory = browserSocket, tabChannelFactory: TabChannelFactory = browserTabChannel) {
...
function useSessionState(services: SessionServices) {
  ...
  const focus = useRef<{ scope: string; saved: NonNullable<ReturnType<typeof captureCommandFocus>> } | null>(null);
  const focusScope = view.phase === "ready"
    ? JSON.stringify([view.identity.actor_type, view.identity.actor_id, view.identity.room_id, view.identity.participant_id]) : "";
  useLayoutEffect(() => {
    if (busy || !focus.current) return;
    const pending = focus.current; focus.current = null;
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      lifecycle.current += 1;
      unsubscribe(); activity.stop(); pendingCheck.current = false;
      releasePendingRecovery.current?.(); releasePendingRecovery.current = null;
      locked.current = false;
      services.recovery.reset();
      focus.current?.saved.cancel(); focus.current = null;
    };
  }, [services, activity]);
  ...
  async function execute(command: () => Promise<unknown>, successMessage = "", changesSession = false, refreshIdentity = true, roomEntry = false): Promise<boolean> {
    const current = services.recovery.getSnapshot();
    const phase = current.phase;
    if (locked.current || (phase === "ready" && current.checking) || (phase !== "ready" && phase !== "anonymous")) return false;
    locked.current = true;
    ...
    let failed = false;
    try { await command(); }
    catch (error) { failed = true; message = failureMessage(error); }
    finally {
      releaseRecovery();
      ...
      if (mounted.current && startedIn === lifecycle.current) {
        ...
        if (refreshIdentity || failed || pendingCheck.current || services.recovery.getSnapshot().phase !== "ready") {
      ...
      if (startedIn === lifecycle.current) {
        locked.current = false;
        if (mounted.current && pendingCheck.current) { pendingCheck.current = false; void services.recovery.recover(); }
      }
    }
  ...
  return {
    ...services, view, busy: busy || (view.phase === "ready" && !!view.checking), blocking, enteringRoom, notice,
    clearNotice: () => setNotice(""), recheck, run: execute,
    ...
    login: (login_id: string, password: string) => execute(
      () => services.api.request("/api/v1/sessions/member", "post", { body: { login_id, password } }),
      "", true,
    ),
    guest: () => execute(() => services.api.request("/api/v1/sessions/guest", "post", {}), "", true),
    logout: () => execute(() => services.api.request("/api/v1/session", "delete", {}), "로그아웃 요청을 처리했습니다.", true),
    register: (login_id: string, nickname: string, password: string) => execute(async () => {
      const member = await services.api.request("/api/v1/members", "post", { body: { login_id, nickname, password } });
      if (!member || !Number.isSafeInteger(member.member_id) || member.member_id < 1
        || member.login_id !== login_id || member.nickname !== nickname.trim()) {
...
export function SessionProvider({ services, children }: { services: SessionServices; children: ReactNode }) {
