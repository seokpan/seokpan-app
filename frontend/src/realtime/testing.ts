...
export class FakeSocket implements Socket {
  ...
  disconnect(code: number) { this.readyState = 3; this.onclose?.call(this as unknown as WebSocket, new CloseEvent("close", { code })); }
}
export function event<T>(type: string, version: number, payload: T, roomId: string | null = null) {
  return { event_type: type, schema_version: 1, event_id: `event-${version}-${type}`, occurred_at: "2026-09-07T00:00:00Z",
    state_version: version, room_id: roomId, game_id: null, payload };
}
