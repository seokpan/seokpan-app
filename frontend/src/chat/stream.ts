...
type ChatView = { phase: "connecting" | "ready" | "closed"; messages: ChatMessage[]; notice: string };
...
export class ChatStream {
  ...
  constructor(private readonly roomId: ChatScope, private readonly factory: SocketFactory) {}
  getSnapshot = () => this.#view;
  subscribe = (listener: () => void) => { this.#listeners.add(listener); return () => { this.#listeners.delete(listener); }; };
  #publish(view: ChatView) { this.#view = view; this.#listeners.forEach(fn => fn()); }
  stop = () => {
    clearTimeout(this.#timer); this.#timer = undefined;
    const socket = this.#socket; this.#socket = null;
    if (socket) { socket.onmessage = null; socket.onclose = null; socket.onerror = null; socket.close(); }
  };
  #close(notice: string) { this.stop(); this.#publish({ phase: "closed", messages: [], notice }); }
  start = () => {
    this.stop();
    this.#publish({ phase: "connecting", messages: [], notice: "연결 이후 대화만 표시합니다. 이전 대화는 보관하지 않습니다." });
  ...
  #connect() {
    ...
    let socket: Socket;
    try { socket = this.factory(this.roomId === null ? "/ws/v1/chat/lobby" : `/ws/v1/chat/rooms/${this.roomId}`); }
    catch { this.#close("채팅 연결을 열 수 없습니다. 다시 연결해 주세요."); return; }
    this.#socket = socket;
    this.#timer = setTimeout(() => this.#close("채팅 연결 응답이 없습니다. 다시 연결해 주세요."), 15_000);
    socket.onmessage = event => {
      ...
      try {
        ...
        if (this.#view.phase === "connecting") {
          ...
          clearTimeout(this.#timer); this.#timer = undefined;
          this.#publish({ ...this.#view, phase: "ready" }); return;
        }
      ...
      } catch { this.#close("채팅 응답을 확인할 수 없어 채팅만 중단했습니다. 다시 연결해 주세요."); }
    };
    socket.onerror = () => { if (socket === this.#socket) this.#close("채팅 연결에 문제가 생겼습니다. 다시 연결해 주세요."); };
    socket.onclose = event => {
      if (socket !== this.#socket) return;
      this.#close([4401, 4403].includes(event.code)
